...
local diag_state = "show"
M.toggle_diag_displ = function(mode) -- show hide off
  local c = vim.diagnostic.config
  if mode == nil then
    if diag_state == "show" then
      diag_state = "off"
    elseif diag_state == "hide" then
      diag_state = "show"
    else
      diag_state = "hide"
    end
  ...
  if diag_state == "off" then
    ...
    if diag_state == "show" then
      ...
      c({ virtual_text = true, update_in_insert = false })
    else
      vim.cmd([[autocmd CursorHold * lua vim.diagnostic.open_float(nil, {focus=false})]])
      c({ virtual_text = false, update_in_insert = false })
    end
    xpcall(vim.diagnostic.enable, function(...)
    end)
  end
  --vim.notify("Diag: " .. diag_state, "debug", { title = "🩺" })
end
return M
